...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Get filter tag buttons
    const filterByTagsButton = document.getElementsByClassName("filter_by_tags")[0];
    const clearFilterButton = document.getElementsByClassName("clear_filter")[0];

    // Get filter input field and filter alert
    const filterInputField = document.getElementsByClassName("filter_inputfield")[0];
    const filterAlertText = document.getElementsByClassName("filter_alert")[0];
    ...
    // Add event listener on filterByTagsButton for filtering posts by tags
    filterByTagsButton.addEventListener("click", filterByTags);

    // Add event listener on clearFilterButton for clearing filters of tags
    clearFilterButton.addEventListener("click", clearFilterTags);   
    
    
    document.addEventListener("keydown", (event) => {

        if(event.key === "Enter" && filterInputField.value != "")
            updateTagSelection(filterInputField);
    ...
    function updateTagSelection(inputElement){
        ...
        if(!exisitingFilterTags.includes(inputValue)){
            ...
            filterAlertText.classList.toggle("filter_alert_hidden");
            filterAlertText.textContent = "Filter already exists!"
    ...
    async function filterByTags(){
        ...
        if(!filterAlertText.classList.contains("filter_alert_hidden"))
            filterAlertText.classList.toggle("filter_alert_hidden");
    ...
    function clearFilterTags(){
        ...
        if(JSON.parse(localStorage.getItem("filter_result")) != null){
            ...
            filterAlertText.textContent = "Please provide input!"
