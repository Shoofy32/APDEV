document.addEventListener("DOMContentLoaded", () => {
    ...
    const all_replies = document.getElementsByClassName("all_replies")[0];
    ...
    // Add event listener on all replies for event delegating

    if(all_replies)
        all_replies.addEventListener("click", async function(event) {
            ...
            // If post reply is clicked, and user is not in userpost and clicked on an edit area, open the post and load userpost.html
                    ...
                    && (window.location.pathname.includes("userprofile") || window.location.pathname.includes("userprofile"))){

                const reply = await fetch(`/reply/${post_reply.id}`); // Fetch from route
                replyInfo = await reply.json() // Convert to object
                openPost(replyInfo.unique_post_id);
